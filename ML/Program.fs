...
let trainingsampleData = File.ReadLines(@"D:\MyData\Projects\ML\ML\trainingsample.csv")
...
let splitLines (lines:seq<string>) =
                lines
                |> Seq.skip 1 
                |> Seq.map (fun line -> line.Split(','))

let linesStr = splitLines trainingsampleData 
...
let linesStrToLinesInt lines = Seq.map (fun str -> (int)str) 
let lines = linesStrToLinesInt linesStr
...
let lineToImage lines = Seq.map (fun (line:int[]) -> { Label = line.[0]; Pixels = line.[1..] })

let images = lineToImage lines
...
let classify (unknown:int[]) =
    ...
    let minDistance = Seq.minBy (fun line -> line.Pixels ) images
    minDistance
...
let linesToImages = splitLines >> linesStrToLinesInt >> lineToImage

let validationLines = linesToImages(File.ReadLines(@"D:\MyData\Projects\ML\ML\validationsample.csv"))
